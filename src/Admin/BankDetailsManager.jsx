...
const Card = ({ children, className = "" }) => (
  <div
    className={
      `rounded-2xl p-5 shadow-[0_0_0_1px_rgba(229,200,112,0.25)] bg-[#111] ${className}`
    }
  >
    {children}
  </div>
...
const Button = ({ children, onClick, type = "button", variant = "primary", disabled }) => {
  ...
  return (
    <button type={type} onClick={onClick} className={classes[variant]} disabled={disabled} style={{ ["--accent"]: ACCENT }}>
      {children}
    </button>
  );
};
...
export default function BankDetailsManager() {
  ...
  const maskAccount = (acc = "") =>
    acc.length <= 4 ? acc : `${"*".repeat(Math.max(0, acc.length - 4))}${acc.slice(-4)}`;
  ...
  const setActive = async (id) => {
    ...
    try {
      ...
      await Promise.all([
        ...
        ...others.map((o) => axios.patch(`${API_BASE}/api/bankdetails/${o._id}`, { isactive: false })),
      ]);
      await fetchAll();
    } catch (e) {
      setError(e?.response?.data?.message || e?.message || "Failed to set active");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen" style={{ background: BG }}>
      <div className="mx-auto max-w-6xl px-4 py-10 text-white">
        <header className="mb-8 flex items-center justify-between">
          <h1 className="text-2xl font-bold tracking-tight">Bank & UPI Details</h1>
          <div className="flex items-center gap-3">
            <Pill tone="default">Total: {items.length}</Pill>
            <Pill tone="active">Active: {items.filter((i) => i.isactive).length}</Pill>
          </div>
        </header>
        ...
        <Card className="mb-10">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <h2 className="text-lg font-semibold mb-2">{editId ? "Update Details" : "Create New Details"}</h2>
              <p className="text-sm text-white/70">Fill bank & UPI info. You can mark a record active after saving, or toggle it from the list.</p>
            </div>
            ...
            <Field label="Account Number" required>
              <input
                ...
                onChange={(e) => onChange("bankdetails.accountnumber", e.target.value.replace(/\s/g, ""))}
                className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[--accent]"
            ...
            <div className="md:col-span-2 flex items-center justify-between gap-3 pt-2">
              <div className="flex items-center gap-3">
                <label className="inline-flex items-center gap-2 select-none">
                  <input
                    ...
                    style={{ ["--accent"]: ACCENT }}
                  />
                  <span className="text-sm text-white/80">Mark this record Active (on save)</span>
                </label>
              </div>
              <div className="flex items-center bg-white gap-3">
                ...
                <Button type="submit" disabled={saving}>{editId ? "Update" : "Create"}</Button>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {loading ? (
            <Card><div className="animate-pulse text-white/70">Loading bank details…</div></Card>
          ) : !hasItems ? (
            <Card>
              <div className="text-white/70">No records yet. Create your first bank/UPI detail above.</div>
            </Card>
          ) : (
            items.map((item) => (
              <Card key={item._id} className="relative">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold">{item.bankdetails?.bankname || "—"}</h3>
                      {item.isactive ? <Pill tone="active">Active</Pill> : <Pill>Inactive</Pill>}
                    </div>
                    <div className="text-sm text-white/80 space-y-1">
                      <div><span className="text-white/50">IFSC:</span> {item.bankdetails?.ifsccode || "—"}</div>
                      <div><span className="text-white/50">A/C:</span> {maskAccount(item.bankdetails?.accountnumber)}</div>
                      <div><span className="text-white/50">Branch:</span> {item.bankdetails?.branch || "—"}</div>
                      <div><span className="text-white/50">UPI:</span> {item.upidetails?.upiid || "—"} ({item.upidetails?.upiname || "—"})</div>
                    </div>
                  </div>
                </div>
                <div className="mt-5 flex items-center gap-3">
                  ...
                  {!item.isactive ? (
                    <Button onClick={() => setActive(item._id)}>Set Active</Button>
                  ) : (
                    <Button variant="ghost" onClick={() => setActive(item._id)}>Make Default Again</Button>
                  )}
                </div>
              </Card>
            ))
        ...
        <footer className="mt-10 text-xs text-white/50">
          Tip: Only one record should be active. When you click <span style={{ color: ACCENT }}>Set Active</span>, others will be toggled off automatically.
        </footer>
      </div>
    </div>
  );
